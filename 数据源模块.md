...
吐槽一点，项目中实际用不太的到动态数据源配置，要核心库就调用核心库就好，有统一的日志中心模块调用log这个数据库，不过这是通用的，你不想用那就不用，但是不能没有   
暂时还不知道怎么实现，看看后面项目模块详解是怎么使用这个模块的    
...
5. 要用到多数据源的话可以使用这个配置文件,然后就是要设置spring.datasource.dynamic.enable = true    
```
spring:
  datasource:
    dynamic:
